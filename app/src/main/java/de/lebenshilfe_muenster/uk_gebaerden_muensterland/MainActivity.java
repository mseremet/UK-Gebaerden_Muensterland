...
public class MainActivity extends AppCompatActivity
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        if (id == R.id.nav_sign_browser) {
            final Fragment signBrowserFragment = new SignBrowserFragment();
            final FragmentManager fragmentManager = getFragmentManager();
            fragmentManager.beginTransaction().replace(R.id.content_frame, signBrowserFragment).commit();
            final ActionBar supportActionBar = getSupportActionBar();
            if (null == supportActionBar)  {
                throw new IllegalStateException("SupportActionBar is null. Should be set in onCreate() method.");
            } else {
                supportActionBar.setTitle(R.string.sign_browser);
            }
